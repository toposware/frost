...
mod dkg_benches {
    ...
    fn round_one_t_out_of_n(c: &mut Criterion) {
        ...
        let mut participants_except_p1 =
            Vec::<Participant>::with_capacity((NUMBER_OF_PARTICIPANTS - 1) as usize);
    ...
    fn round_two_t_out_of_n(c: &mut Criterion) {
        ...
        let mut p1_my_secret_shares =
            Vec::<SecretShare>::with_capacity((NUMBER_OF_PARTICIPANTS - 1) as usize);
        ...
        for i in 2..NUMBER_OF_PARTICIPANTS + 1 {
            ...
            let pi_state = DistributedKeyGeneration::<_>::new(
                &params,
...
mod sign_benches {
    ...
    fn partial_sign_t_out_of_n(c: &mut Criterion) {
        ...
        let mut participants_secret_shares: Vec<Vec<SecretShare>> = (0..NUMBER_OF_PARTICIPANTS)
        ...
        participants_states_2.push(
            participants_states_1[0]
                .clone()
                .to_round_two(p1_my_secret_shares)
                .unwrap(),
        );

        for i in 2..NUMBER_OF_PARTICIPANTS + 1 {
            let mut pi_my_secret_shares =
                Vec::<SecretShare>::with_capacity((NUMBER_OF_PARTICIPANTS - 1) as usize);
            let mut increment = -1i32;
            for j in 1..i {
                pi_my_secret_shares.push(
                    participants_secret_shares[(j - 1) as usize]
                        [((i - j) as i32 + increment) as usize]
                        .clone(),
                );
                increment += 1;
            }
            for j in (i + 1)..NUMBER_OF_PARTICIPANTS + 1 {
                pi_my_secret_shares
                    .push(participants_secret_shares[(j - 1) as usize][(i - 1) as usize].clone());
            }

            participants_states_2.push(
                participants_states_1[(i - 1) as usize]
                    .clone()
                    .to_round_two(pi_my_secret_shares)
                    .unwrap(),
            );
        }
        ...
        let (group_key, p1_sk) = participants_states_2[0]
            .clone()
            .finish(participants[1].public_key().unwrap())
            .unwrap();
        ...
        for i in 2..THRESHOLD_OF_PARTICIPANTS + 1 {
            let (_, pi_sk) = participants_states_2[(i - 1) as usize]
                .clone()
                .finish(participants[(i - 1) as usize].public_key().unwrap())
                .unwrap();
